...
func _rebuild_city() -> void:
    if is_instance_valid(city_arena):
        city_arena.configure(GlobalMap.selected_country, OperationData.get_operation(GlobalMap.selected_country, operation_index)[0], OperationData.get_operation(GlobalMap.selected_country, operation_index)[1])
        city_arena.build()
...
func _spawn_enemies() -> void:
    var positions := [Vector3(-20,1,-20), Vector3(20,1,-20), Vector3(-30,1,15), Vector3(30,1,5), Vector3(0,1,-30)]
    for p in positions:
...
func _build_hud() -> void:
    ...
    hud.name = "HUD"
    add_child(hud)
    score_label = Label.new(); score_label.position = Vector2(28, 18); score_label.add_theme_font_size_override("font_size", 28); score_label.text = "SKOR 0"; hud.add_child(score_label)
    status_label = Label.new(); status_label.position = Vector2(28, 55); status_label.add_theme_font_size_override("font_size", 18); hud.add_child(status_label)
    mission_label = Label.new(); mission_label.position = Vector2(28, 115); mission_label.add_theme_font_size_override("font_size", 17); hud.add_child(mission_label)
    health_label = Label.new(); health_label.position = Vector2(28, 145); health_label.add_theme_font_size_override("font_size", 22); hud.add_child(health_label)
    ammo_label = Label.new(); ammo_label.position = Vector2(1050, 30); ammo_label.add_theme_font_size_override("font_size", 24); hud.add_child(ammo_label)
    crosshair = Label.new(); crosshair.text = "+"; crosshair.position = Vector2(632, 330); crosshair.add_theme_font_size_override("font_size", 28); hud.add_child(crosshair)
    var map_button := Button.new(); map_button.text = "DÜNYA / ÜLKE SEÇ"; map_button.position = Vector2(28, 190); map_button.size = Vector2(210, 55); map_button.pressed.connect(_toggle_world_map); hud.add_child(map_button)
    ...
    map_label = Label.new(); map_label.position = Vector2(25, 18); map_label.add_theme_font_size_override("font_size", 22); map_panel.add_child(map_label)
    ...
    _refresh_country_list(); _update_voice_button();
    ...
    var fire := Button.new(); fire.text = "ATEŞ"; fire.position = Vector2(1080, 545); fire.size = Vector2(150, 100); fire.add_theme_font_size_override("font_size", 24); fire.button_down.connect(func(): player.touch_fire = true); fire.button_up.connect(func(): player.touch_fire = false); hud.add_child(fire)
    ...
    var hint := Label.new(); hint.position = Vector2(28, 675); hint.text = "SOL: hareket • SAĞ EKRAN: nişan • ATEŞ • ŞARJÖR"; hint.add_theme_font_size_override("font_size", 16); hud.add_child(hint)
...
func _refresh_country_list() -> void:
    ...
    for continent in continents:
        var header := Label.new(); header.text = "— %s —" % continent; header.add_theme_font_size_override("font_size", 18); country_list.add_child(header)
...
func _refresh_operation_list() -> void:
    ...
    var header := Label.new(); header.text = "ŞEHİR / OPERASYON"; header.add_theme_font_size_override("font_size", 18); operation_list.add_child(header)
    ...
    for i in range(operations.size()):
        ...
        var button := Button.new(); button.text = "%s • %s • Zorluk %d" % [item[0], item[1], item[3]]; button.alignment = HORIZONTAL_ALIGNMENT_LEFT; button.pressed.connect(func(): _select_operation(i)); operation_list.add_child(button)
...
func _add_hold_button(hud: CanvasLayer, text: String, pos: Vector2, action: String) -> void:
    var button := Button.new(); button.text = text; button.position = pos; button.size = Vector2(80, 80); button.add_theme_font_size_override("font_size", 28); button.button_down.connect(func(): Input.action_press(action)); button.button_up.connect(func(): Input.action_release(action)); hud.add_child(button)
...
func _on_enemy_died() -> void:
    ...
    if score >= 500:
        ...
        if map_open: _refresh_country_list()
