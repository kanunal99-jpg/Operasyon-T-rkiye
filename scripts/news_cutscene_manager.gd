...
func _ready() -> void:
    ...
    add_child(panel)
    category_label = Label.new(); category_label.position = Vector2(35, 28); category_label.add_theme_font_size_override("font_size", 20); panel.add_child(category_label)
    headline_label = Label.new(); headline_label.position = Vector2(35, 75); headline_label.size = Vector2(1070, 90); headline_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; headline_label.add_theme_font_size_override("font_size", 34); panel.add_child(headline_label)
    body_label = Label.new(); body_label.position = Vector2(35, 180); body_label.size = Vector2(1070, 230); body_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART; body_label.add_theme_font_size_override("font_size", 22); panel.add_child(body_label)
    location_label = Label.new(); location_label.position = Vector2(35, 430); location_label.size = Vector2(700, 40); panel.add_child(location_label)
    progress_label = Label.new(); progress_label.position = Vector2(35, 475); panel.add_child(progress_label)
    continue_button = Button.new(); continue_button.text = "DEVAM ET"; continue_button.position = Vector2(820, 455); continue_button.size = Vector2(140, 60); continue_button.pressed.connect(_next_card); panel.add_child(continue_button)
    skip_button = Button.new(); skip_button.text = "⏭ ATLA"; skip_button.position = Vector2(970, 455); skip_button.size = Vector2(140, 60); skip_button.pressed.connect(skip); panel.add_child(skip_button)
    panel.visible = false
