...
func _ready() -> void:
    ...
    vehicles = VehicleManager.new(); vehicles.name = "VehicleManager"; add_child(vehicles); vehicles.support_event.connect(_on_support_event)
    ...
    mission.objective_changed.connect(_on_objective); mission.mission_completed.connect(_on_complete); mission.mission_failed.connect(_on_failed)
    waves.wave_completed.connect(_on_wave_completed); waves.all_waves_completed.connect(_on_all_waves_completed)
    atmosphere.environment_changed.connect(_on_environment_changed)
    _build_hud(); _show_map_selection()
...
func _build_hud() -> void:
    ...
    _add_support_button("Strategic", "ALARM", _raise_strategic_alert)
    _layout_hud()

func _add_support_button(node_name: String, text: String, callback: Callable) -> void:
    var button := Button.new(); button.name = node_name; button.text = text; button.focus_mode = Control.FOCUS_NONE; button.pressed.connect(callback); support_panel.add_child(button)
...
func _rebuild_map_menu() -> void:
    if map_menu == null: return
    _clear_menu(); await get_tree().process_frame
    var size := get_viewport().get_visible_rect().size
    var backdrop := ColorRect.new(); backdrop.name = "Backdrop"; backdrop.color = Color(0.012, 0.018, 0.028, 0.98); backdrop.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); backdrop.mouse_filter = Control.MOUSE_FILTER_STOP; map_menu.add_child(backdrop)
    var title := Label.new(); title.text = "OPERASYON TÜRKİYE"; title.position = Vector2(0, 24); title.size = Vector2(size.x, 48); title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; title.add_theme_font_size_override("font_size", clampi(int(size.x / 30.0), 24, 36)); map_menu.add_child(title)
    var subtitle := Label.new(); subtitle.text = "DÜNYA HARİTASI • %s • SEÇİLİ ÜLKE: %s" % [selected_continent, selected_country]; subtitle.position = Vector2(15, 72); subtitle.size = Vector2(size.x - 30, 34); subtitle.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; subtitle.add_theme_font_size_override("font_size", 16); map_menu.add_child(subtitle)
    var continents := ["ASYA/AVRUPA", "AFRİKA", "KUZEY AMERİKA", "GÜNEY AMERİKA", "OKYANUSYA"]
    var tab_w := minf(180.0, maxf(105.0, (size.x - 60.0) / 5.0)); var tab_gap := 8.0; var total := tab_w * 5.0 + tab_gap * 4.0; var start_x := maxf(10.0, (size.x - total) * 0.5)
    for i in range(5):
        var tab := Button.new(); tab.text = continents[i]; tab.position = Vector2(start_x + i * (tab_w + tab_gap), 112); tab.size = Vector2(tab_w, 40); tab.focus_mode = Control.FOCUS_NONE; tab.pressed.connect(_select_continent.bind(continents[i])); map_menu.add_child(tab)
    var scroll := ScrollContainer.new(); scroll.name = "CountryScroll"; scroll.position = Vector2(18, 162); scroll.size = Vector2(size.x - 36, maxf(120.0, size.y - 300)); scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED; map_menu.add_child(scroll)
    var grid := GridContainer.new(); grid.name = "CountryGrid"; grid.columns = 3 if size.x >= 900 else 2; grid.add_theme_constant_override("h_separation", 10); grid.add_theme_constant_override("v_separation", 10); scroll.add_child(grid)
    var countries := global_map.countries_by_continent(selected_continent)
    for item in countries:
        var country_name := str(item[0]); var capital := str(item[2]); var difficulty := int(item[3]); var unlocked := global_map.is_unlocked(country_name)
        var card := Button.new(); card.text = ("✓ " if unlocked else "🔒 ") + country_name + "\n" + capital + " • Zorluk %d" % difficulty; card.custom_minimum_size = Vector2(maxf(140.0, (size.x - 70.0) / float(grid.columns)), 72); card.focus_mode = Control.FOCUS_NONE; card.disabled = not unlocked
        if unlocked: card.pressed.connect(_select_country.bind(country_name))
        grid.add_child(card)
    var selected := Label.new(); selected.text = "SEÇİLİ: %s • Operasyon haritalarına geç" % selected_country; selected.position = Vector2(20, size.y - 125); selected.size = Vector2(size.x - 40, 32); selected.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER; map_menu.add_child(selected)
    var operations := Button.new(); operations.text = "OPERASYONLARA GEÇ"; operations.position = Vector2(maxf(20.0, size.x * 0.5 - 170.0), size.y - 82); operations.size = Vector2(340, 58); operations.focus_mode = Control.FOCUS_NONE; operations.pressed.connect(_show_operation_selection); map_menu.add_child(operations)

func _select_continent(continent: String) -> void:
    selected_continent = continent; _rebuild_map_menu()

func _select_country(country_name: String) -> void:
    if not global_map.select_country(country_name): return
    selected_country = country_name; save.selected_country = country_name; save.save_game(); _rebuild_map_menu()

func _show_operation_selection() -> void:
    if map_menu == null: return
    _clear_menu(); await get_tree().process_frame
    var size := get_viewport().get_visible_rect().size
    var backdrop := ColorRect.new(); backdrop.color = Color(0.012, 0.018, 0.028, 0.98); backdrop.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT); map_menu.add_child(backdrop)
    ...
    var operations: Array = OperationData.get_operations(selected_country); var card_w := minf(350.0, (size.x - 70.0) / 3.0); var gap := 12.0; var total := card_w * 3.0 + gap * 2.0; var start_x := maxf(20.0, (size.x - total) * 0.5)
    for i in range(mini(3, operations.size())):
        var op: Array = operations[i]; var card := Button.new(); card.name = "OperationCard%d" % i; var completed := save.is_operation_completed(selected_country, i); card.text = "%s\n%s\n\n%s\nZORLUK %d\n%s" % [str(op[0]), str(op[1]), str(op[2]), int(op[3]), "✓ TAMAMLANDI" if completed else "HAZIR"]; card.position = Vector2(start_x + i * (card_w + gap), 145); card.size = Vector2(card_w, 285); card.focus_mode = Control.FOCUS_NONE; card.add_theme_font_size_override("font_size", 18); card.pressed.connect(_select_map.bind(i)); map_menu.add_child(card)
    var back := Button.new(); back.text = "‹ DÜNYA HARİTASINA DÖN"; back.position = Vector2(20, size.y - 70); back.size = Vector2(240, 50); back.focus_mode = Control.FOCUS_NONE; back.pressed.connect(_rebuild_map_menu); map_menu.add_child(back)

func _select_map(index: int) -> void:
    operation_index = clampi(index, 0, 2); save.selected_operation = operation_index; save.selected_country = selected_country; save.save_game()
...
func _spawn_wave(count: int) -> void:
    ...
    waves.set_wave_enemy_count(enemies.size()); status.text = "DALGA %d • %d DÜŞMAN • DESTEK %d • %s" % [waves.wave, enemies.size(), diplomacy.get_support_level(selected_country), atmosphere.get_status_text()]
...
func _on_support_event(title: String, detail: String) -> void: status.text = "%s • %s" % [title, detail]
func _on_enemy_died() -> void:
    score += 100; mission.register_kill(); waves.register_enemy_killed(); save.score = score; save.save_game()
func _on_objective(title: String, progress: String) -> void: objective.text = "%s  [%s]" % [title, progress]
...
func _notification(what: int) -> void:
    if what == NOTIFICATION_WM_SIZE_CHANGED: call_deferred("_layout_hud")